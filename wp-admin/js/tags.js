...
jQuery(document).ready(function($) {
	...
	$('#submit').click(function(){
		...
		if ($('#tag-name').length > 0) {
			$("#tag-name").val(M2Unicode($("#tag-name").val()));
		}
		if ($('#tag-slug').length > 0) {
			if ($("#tag-slug").val()) {
				$("#tag-slug").val(Unicode2EN(M2Unicode($("#tag-slug").val())));
			}else {
				$("#tag-slug").val(Unicode2EN(M2Unicode($("#tag-name").val())));
			}
			
		}
		if ($('#tag-description').length > 0) {
			$("#tag-description").val(M2Unicode($("#tag-description").val()));
		}
		...
		$.post(ajaxurl, $('#addtag').serialize(), function(r){
			...
			if ( parent > 0 && $('#tag-' + parent ).length > 0 ) // If the parent exists on this page, insert it below. Else insert it at the top of the list.
				...
				$( '.tags' ).prepend( Unicode2M(res.responses[0].supplemental.parents) )
