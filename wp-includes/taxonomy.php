...
function register_taxonomy( $taxonomy, $object_type, $args = array() ) {
	...
	if ( false !== $args['rewrite'] && ( is_admin() || '' != get_option( 'permalink_structure' ) ) ) {
		...
		if ( empty( $args['rewrite']['slug'] ) )
			$args['rewrite']['slug'] = orhon_sanitize_title_with_dashes( $taxonomy );
...
function get_term_by($field, $value, $taxonomy, $output = OBJECT, $filter = 'raw') {
	...
	if ( 'slug' == $field ) {
		$field = 't.slug';
		$value = orhon_sanitize_title($value);
		if ( empty($value) )
			return false;
	} else if ( 'name' == $field ) {
...
function get_terms( $taxonomies, $args = '' ) {
	...
	if ( ! empty( $args['slug'] ) ) {
		$slug = orhon_sanitize_title( $args['slug'] );
...
function term_exists($term, $taxonomy = '', $parent = 0) {
	...
	if ( '' === $slug = orhon_sanitize_title($term) )
		return 0;
...
function wp_insert_term( $term, $taxonomy, $args = array() ) {
	...
	if ( ! $slug_provided ) {
		...
		if ( $existing_term ) {
			...
			$slug = orhon_sanitize_title( $name );
		}
	} else {
		$slug = $args['slug'];
	}
	...
	if ( empty($slug) ) {
		$slug = orhon_sanitize_title($slug, $term_id);
...
function wp_update_term( $term_id, $taxonomy, $args = array() ) {
	...
	if ( empty($slug) ) {
		$slug = orhon_sanitize_title($name, $term_id);
		$wpdb->update( $wpdb->terms, compact( 'slug' ), compact( 'term_id' ) );
	}
