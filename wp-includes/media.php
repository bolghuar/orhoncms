...
function wp_video_shortcode( $attr, $content = '' ) {
	...
	$width_rule = $height_rule = $orhon_width_rule = $orhon_height_rule ='';
	if ( ! empty( $atts['width'] ) ) {
		$width_rule = sprintf( 'width: %dpx; ', $atts['width'] );
		$orhon_width_rule = sprintf( 'height: %dpx; ', $atts['width'] );
	}
	if ( ! empty( $atts['height'] ) ) {
		$height_rule = sprintf( 'height: %dpx; ', $atts['height'] );
		$orhon_height_rule = sprintf( 'width: %dpx; ', $atts['height'] );
	}
	$output = sprintf( '<div style="%s%s" class="orhon-wp-video"><div style="%s%s" class="wp-video">%s</div></div>',$orhon_height_rule ,$orhon_width_rule,$width_rule, $height_rule, $html );
