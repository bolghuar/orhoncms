...
tinymce.PluginManager.add( 'wpeditimage', function( editor ) {
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:wp_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:wp_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			return '<div class="mceTemp"><dl id="' + id + '" class="wp-caption ' + align + classes + '" style="height: ' + width + 'px">' +
	...
	function updateImage( imageNode, imageData ) {
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				...
				html =  '<dl ' + id + 'class="' + className +'" style="height: '+ width +'px">' +
			...
			if ( ! imgNode ) {
				...
				if ( caption ) {
					...
					if ( data.width ) {
						...
						captionWidth = ' style="height: ' + captionWidth + 'px"';
					}
